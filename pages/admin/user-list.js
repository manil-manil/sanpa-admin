...
function UserList() {
  ...
  const fetchUserList = () => {
    fetchWrapper.get(`${BASIC_CONSTANT.BACKEND_URL}/api/user/admin`)
    .then(response=>{
      setRows(response);
  ...
  const deleteUser = React.useCallback(
    (id) => () => {
      setTimeout(() => {
        if(confirm('삭제하시겠습니까?')){
          fetchWrapper.delete(`${BASIC_CONSTANT.BACKEND_URL}/api/user/admin${id}`)
          .then(response=>{
            fetchUserList()
          }).catch(err=>{console.log(err)})
        }
  ...
  const handleClickOpen = React.useCallback(
    (id) => () => {
      setTimeout(() => {
        fetchWrapper.get(`${BASIC_CONSTANT.BACKEND_URL}/api/user/admin/${id}`)
        .then(response=>{
          setInputs(response);
          setOpen(true);
        }).catch(err=>{console.log(err)})
  ...
  const handleClose = () => {//회원수정 모달 닫기
    setOpen(false);
  }
  const handleSave = () => {
    //회원수정api호출
    fetchWrapper.put(`${BASIC_CONSTANT.BACKEND_URL}/api/user/admin/${inputs.userId}`,{
  ...
  const columns = React.useMemo( 
   () => [
    ...
    },
    {
      ...
      headerName: '관리',
      getActions: (params) => [
        <GridActionsCellItem icon={<EditIcon/>} onClick={handleClickOpen(params.id)}  label="Edit" />,
